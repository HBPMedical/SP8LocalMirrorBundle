...
#Input Output
Input: RAW uses as input the files generated by MIPMap. It supports the following data formats:
   * CSV
   * JSON
   * XML
   * Text and log files
Output: A REST service with the query results in JSON format. The specification of the REST interface can be found in the appendix in section Error: Reference source not found.
